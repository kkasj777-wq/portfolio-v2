...
OUTPUT = ROOT / "output" / "pdf" / "Wang-Chenxin-Author-Director-Portfolio-2026-V5.pdf"
SPEC_OUTPUT = ROOT / "output" / "pdf" / "Wang-Chenxin-Portfolio-V5-Design-Spec.pdf"
...
ICE = HexColor("#A9C8DD")
NIGHT = HexColor("#1A2A3A")
STONE = HexColor("#6B7B8D")
STONE_LIGHT = HexColor("#C8D8E8")
HAND_LIGHT = HexColor("#F0F4F8")
RIVER = HexColor("#4A6572")
FOG = HexColor("#A3B1C6")
CREAM = HexColor("#FFF3E0")
PEACH = HexColor("#FFDAB9")
PAPER = HexColor("#E8ECEF")
TECH = HexColor("#2C3E50")
TECH_LIGHT = HexColor("#9CB4C5")
...
def warm_image(path_string: str) -> Path:
    source = Path(path_string)
    stat = source.stat()
    signature = f"{source.resolve()}|{stat.st_mtime_ns}|{stat.st_size}|warm-v5".encode("utf-8")
    target = CACHE / f"{hashlib.sha1(signature).hexdigest()[:24]}-warm.jpg"
    if target.exists():
        return target
    CACHE.mkdir(parents=True, exist_ok=True)
    with Image.open(source) as raw:
        ...
        overlay = Image.new("RGB", image.size, (255, 239, 222))
...
class Portfolio:
    def __init__(self, output: Path):
        ...
        self.c.setTitle("Wang Chenxin - Author-Director Portfolio - 2026 - V5")
    ...
    def page_number(self, bg: Color = BG) -> None:
        luminance = 0.2126 * bg.red + 0.7152 * bg.green + 0.0722 * bg.blue
        self.c.setFillColor(WHITE if luminance < 0.5 else NOTE)
    ...
    def image_cover(self, path: Path, x: float, y: float, width: float, height: float, anchor_x: float = 0.5,
                    anchor_y: float = 0.5, treatment: str = "cool") -> None:
        target = warm_image(str(path)) if treatment == "warm" else cool_image(str(path))
    ...
    def image_contain(self, path: Path, x: float, y: float, width: float, height: float, bg: Color = BG,
                      treatment: str = "cool") -> None:
        target = warm_image(str(path)) if treatment == "warm" else cool_image(str(path))
        iw, ih = image_size(str(target))
...
def draw_page_1(b: Portfolio) -> None:
    b.new(NIGHT, footer=False)
    top_h = PAGE_H / 3
    b.c.setFillColor(NIGHT)
...
def draw_page_5(b: Portfolio) -> None:
    b.new(STONE)
    b.title("崖佛不语，岁岁佑我", gx(0), 320, gw(12), 48, WHITE, TA_CENTER)
    b.para("YAFO BUYU, SUISUI YOU WO / DIRECTING CASE 01", gx(2), 252, gw(8), 10, 14, STONE_LIGHT, False, TA_CENTER)
    b.line(gx(2), 116, gw(8), STONE_LIGHT, 0.8)
...
def draw_page_6(b: Portfolio) -> None:
    ...
    b.label("CHILDHOOD / ADULT / RETURN", img_x + img_w - 164, 196, STONE, 8)
    b.title("导演笔记", gx(2), 166, gw(2), 16, STONE)
    b.para("石刻不是背景，而是人物记忆的空间。镜头先让人物被场域包围，再逐步靠近，使童年祈愿与成年回望彼此照亮。", gx(4), 168, gw(6), 11, 18, NIGHT)


def draw_page_7(b: Portfolio) -> None:
    b.new(STONE_LIGHT)
    b.title("STONE / RITUAL / OBJECT", gx(0), PAGE_H - M + 6, gw(8), 24, STONE)
    items = [
        ("STONE", "石刻提供比人物更长的时间尺度。", HexColor("#DCE5ED")),
        ("RITUAL", "祈愿动作把守护从仪式转为关系。", BG),
        ("OBJECT", "苹果与陪伴连接童年和成年。", HexColor("#B8C8D6")),
    ]
...
def draw_page_8(b: Portfolio) -> None:
    b.new(STONE)
    b.title("守护不是答案，\n是被时间重新看见。", gx(1), 342, gw(10), 36, WHITE, TA_CENTER)
    b.line(gx(4), 180, gw(4), STONE_LIGHT, 0.7)
...
def draw_page_9(b: Portfolio) -> None:
    ...
    b.title("容器", gx(0) + 22, 422, left_w - 44, 24, WHITE)
    b.para("大足石刻被作为关系的容器，而不是地域标签。冷灰雨雾、石壁尺度和人物表情共同完成情感回声。", gx(0) + 22, 360, left_w - 44, 10, 17, WHITE)
...
def draw_page_11(b: Portfolio) -> None:
    ...
    for i in range(steps):
        t = i / (steps - 1)
        r = NIGHT.red * (1 - t) + HAND_LIGHT.red * t
        g = NIGHT.green * (1 - t) + HAND_LIGHT.green * t
        bl = NIGHT.blue * (1 - t) + HAND_LIGHT.blue * t
...
def draw_page_13(b: Portfolio) -> None:
    b.new(HAND_LIGHT)
    b.title("176 个镜头判断", gx(0), PAGE_H - M + 6, gw(6), 24, NIGHT)
    points = []
    for i in range(18):
        x = gx(0) + i * gw(12) / 17
...
def draw_page_14(b: Portfolio) -> None:
    ...
    b.title("当光照回彼此，\n关系才真正出现。", gx(2), 338, gw(8), 36, WHITE, TA_CENTER)


def draw_page_15(b: Portfolio) -> None:
    b.new(RIVER, footer=False)
...
def draw_page_16(b: Portfolio) -> None:
    b.new(FOG)
    b.title("七个单元，一张山城地图", gx(0), PAGE_H - M + 6, gw(7), 24, RIVER)
    b.c.setStrokeColor(HexColor("#DCE4EA"))
    b.c.setLineWidth(1)
    for k in range(6):
        p = b.c.beginPath()
        for i in range(80):
    ...
    for i, (name, (col, y)) in enumerate(zip(episodes, positions), 1):
        x = gx(col)
        b.c.setFillColor(HexColor("#EAF0F3"))
        b.c.setStrokeColor(RIVER)
        b.c.rect(x, y, 142, 28, fill=1, stroke=1)
        b.c.setFillColor(RIVER)
...
def draw_page_17(b: Portfolio) -> None:
    b.new(HexColor("#DDE6EB"))
    b.title("镜头不是生成之后\n才决定的。", gx(0), PAGE_H - M + 2, gw(4), 24, RIVER)
...
def draw_page_18(b: Portfolio) -> None:
    b.new(FOG)
    b.title("连续性来自资产系统", gx(0), PAGE_H - M + 6, gw(7), 24, RIVER)
    assets = [
    ...
    for i, (label, image) in enumerate(assets):
        x = gx(i * 3)
        w = gw(3)
        b.card(x, 124, w, 316, HexColor("#EAF0F3"), RIVER)
...
def draw_page_19(b: Portfolio) -> None:
    ...
    b.label("SHOT DIRECTIONS / EPISODE 01", gx(0), 382, FOG, 9)
...
def draw_bread_icon(c: canvas.Canvas, cx: float, cy: float, scale: float = 1.0, color: Color = ICE) -> None:
...
def draw_page_22(b: Portfolio) -> None:
    b.new(CREAM, footer=False)
    b.c.setFillColor(PEACH)
    b.c.circle(PAGE_W / 2, 352, 86, fill=1, stroke=0)
    draw_bread_icon(b.c, PAGE_W / 2, 352, 1.2, RIVER)
    b.title("麦麦的魔法面包店", gx(1), 260, gw(10), 36, RIVER, TA_CENTER)
    b.para("MAIMAI'S MAGIC BAKERY / 15 EPISODES", gx(2), 202, gw(8), 10, 14, MIST, False, TA_CENTER)


def draw_page_23(b: Portfolio) -> None:
    b.new(CREAM)
    warm_text = HexColor("#69544D")
    b.title("重复的是单元机制，\n不重复每一集的视觉答案。", gx(0), PAGE_H - M + 2, gw(5), 24, warm_text)
    b.para("每集由一种魔法面包触发一次选择。共同的店铺和角色建立安全感，能力、场景和冲突保留差异。", gx(0), 316, gw(5), 11, 18, warm_text)
    b.image_cover(s("maimai/ep03.jpg"), gx(5), 106, gw(7), 372, treatment="warm")
...
def draw_page_24(b: Portfolio) -> None:
    ...
    width = PAGE_W * 0.80
    x = (PAGE_W - width) / 2
    b.image_cover(s("maimai/ep14.jpg"), x, 150, width, 330, treatment="warm")
    b.para("奇幻机制把抽象困惑转为孩子能够行动、选择和承担结果的体验。", x, 122, width, 10, 15, HexColor("#7A5E55"))


def draw_page_25(b: Portfolio) -> None:
    b.new(CREAM)
    b.title("场景修改，让差异变得可见。", gx(0), PAGE_H - M + 6, gw(8), 24, HexColor("#69544D"))
    ...
    b.image_cover(before, gx(0), 150, gw(5), 282, treatment="warm")
    b.image_cover(after, gx(7), 150, gw(5), 282, treatment="warm")
    b.label("BEFORE", gx(0), 126, HexColor("#9A7164"))
    b.label("AFTER", gx(7), 126, HexColor("#9A7164"))
    b.arrow(gx(5) + 10, 290, gx(7) - 10, 290, HexColor("#B98270"))


def draw_page_26(b: Portfolio) -> None:
    b.new(PEACH)
    b.title("每一个魔法，\n都要回到一次选择。", gx(1), 410, gw(10), 32, HexColor("#69544D"), TA_CENTER)
    nodes = [(PAGE_W / 2, 244, "MAGIC"), (PAGE_W / 2 + 150, 164, "CHOICE"), (PAGE_W / 2 - 150, 164, "RESULT")]
    for x, y, label in nodes:
        b.c.setFillColor(CREAM)
        b.c.setStrokeColor(HexColor("#B98270"))
        b.c.circle(x, y, 48, fill=1, stroke=1)
        b.c.setFillColor(HexColor("#69544D"))
        b.c.setFont(EN_BOLD, 10)
        b.c.drawCentredString(x, y - 3, label)
    warm_line = HexColor("#B98270")
    b.arrow(nodes[0][0] + 38, nodes[0][1] - 20, nodes[1][0] - 40, nodes[1][1] + 18, warm_line)
    b.arrow(nodes[1][0] - 40, nodes[1][1] - 30, nodes[2][0] + 40, nodes[2][1] - 30, warm_line)
    b.arrow(nodes[2][0] + 12, nodes[2][1] + 45, nodes[0][0] - 12, nodes[0][1] - 45, warm_line)
...
def draw_page_28(b: Portfolio) -> None:
    b.new(TECH)
    b.title("AI 不是创作者身份的替代品。", gx(0), PAGE_H - M + 2, gw(5), 24, WHITE)
    b.para("人物动机、世界规则、镜头判断和最终取舍始终由导演负责。现有归档没有保留工具界面截图，因此本页使用真实分镜与生成资产呈现工作流程，不伪造软件界面。", gx(0), 330, gw(5), 11, 18, TECH_LIGHT)
    b.card(gx(5), 112, gw(7), 364, HexColor("#354B60"), TECH_LIGHT)
    board = f(r"重庆故事\《通幽录》第一集项目交付包\4分镜头脚本\4.2图示分镜头\回家分镜\到家2.png")
    b.image_contain(board, gx(5) + 12, 244, gw(7) - 24, 216, HexColor("#354B60"))
    b.image_cover(s("maimai/ep03.jpg"), gx(5) + 12, 128, gw(3), 96)
    b.image_cover(s("tongyoulu/ep03.jpg"), gx(8) + 4, 128, gw(4) - 16, 96)
    b.label("VERIFIED PROCESS ASSETS", gx(5) + 12, 116, TECH_LIGHT)


def draw_page_29(b: Portfolio) -> None:
    b.new(TECH)
    b.title("实验影像不是逃逸，\n而是寻找新的叙事形式。", gx(0), PAGE_H - M + 2, gw(5), 24, WHITE)
    b.para("志怪、民俗、非遗与地域经验被重新组织为影像规则。", gx(0), 300, gw(5), 11, 18, TECH_LIGHT)
    b.image_cover(clean("jinyan_04.jpg"), gx(5), 110, gw(7), 368)


def draw_page_30(b: Portfolio) -> None:
    b.new(TECH)
    b.title("真实协作，让署名边界\n成为创作的一部分。", gx(0), PAGE_H - M + 2, gw(7), 24, WHITE)
    images = [clean("field_00.jpg"), clean("field_04.jpg"), clean("field_05.jpg")]
    for i, image in enumerate(images):
        b.image_cover(image, gx(i * 4), 190, gw(4), 210)
    b.para("共青团伙伴计划纪录短片 / 出品、策划：共青团重庆市綦江区委员会 / 王陈鑫：摄影、剪辑；摄影为共同署名。", gx(0), 148, gw(12), 10, 16, TECH_LIGHT)


def draw_page_31(b: Portfolio) -> None:
    b.new(TECH)
    b.title("摄影最终回到一个导演问题：\n观众先看见什么？", gx(1), PAGE_H - M + 2, gw(10), 24, WHITE, TA_CENTER)
    images = [
    ...
    for i, (image, label) in enumerate(zip(images, labels)):
        ...
        b.label(label, x, 132, TECH_LIGHT)
        if i < 2:
            b.arrow(x + gw(4) + 3, 264, gx((i + 1) * 4) - 3, 264, TECH_LIGHT)
...
def build_spec() -> None:
    ...
    b.new(BG)
    b.title("PORTFOLIO V5 / NARRATIVE COLOR SYSTEM", gx(0), PAGE_H - M + 6, gw(10), 24, NAVY)
    b.para("不同板块拥有独立主题色；12 栏网格、字体层级与图片比例维持全书连贯。中性背景统一使用 #F5F7FA。", gx(0), 430, gw(8), 11, 18, TEXT)
    b.para("章节比例：主色约 60% / 中性色约 30% / 强调色约 10%；单一板块不超过 3 种颜色。", gx(0), 394, gw(8), 9, 14, NOTE)
    palette = [("COVER / NIGHT", NIGHT, "#1A2A3A"), ("YAFO / STONE", STONE, "#6B7B8D"), ("TONGYOU / RIVER", RIVER, "#4A6572"), ("MAIMAI / CREAM", CREAM, "#FFF3E0"), ("AI / TECH", TECH, "#2C3E50")]
    for i, (label, color, value) in enumerate(palette):
        x = gx(i * 2)
        b.c.setFillColor(color)
        b.c.rect(x, 246, gw(2), 82, fill=1, stroke=0)
        b.label(label, x, 226, NAVY, 7)
    ...
    b.para("冷色章节：RGB / 150 dpi / 提亮暗部 / 对比度 +7% / 饱和度 -34% / 蓝青叠加 13%。麦麦章节：奶油黄与蜜桃粉页面基底，图片轻度提亮并保留治愈感。", gx(6) + 18, 344, gw(6) - 36, 11, 18, TEXT)
    b.para("交付：34 页可复制文字 PDF + V5 叙事色彩规范 PDF。", gx(0), 126, gw(6), 11, 18, NAVY, True)
    b.para("色彩节奏：暗夜蓝 → 冷灰白 → 青石灰 → 冷白与暗夜蓝 → 江水青 → 奶油黄与蜜桃粉 → 科技灰蓝 → 冷灰白。", gx(0), 86, gw(12), 8, 12, NOTE)
    b.finish()
